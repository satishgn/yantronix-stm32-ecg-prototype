...
static void LCD_BackLightConfig( void )
{
    ...
    /* -----------------------------------------------------------------------------------
    TIM2 Configuration: generate 1 PWM signal with variable Duty cycle on CH2.
    ...
    TIM2 Frequency = TIM3 counter clock / (ARR + 1)
                   = 100000 / 100 = 1000 Hz
    TIM2 Channel2 duty cycle = (TIM2_CCR2 / (TIM2_ARR + 1)) * 100 = (50 / 100) * 100 = 50 %
    ------------------------------------------------------------------------------------- */
...
{
    uint16_t CCR2_Value = ((Brightness * 99) / 100); //Brightness = Duty_Cycle, TIM2_Autoreload = 99
    ...
    if(Brightness)
    {
        ...
        TIM_SetCompare2(TIM2, CCR2_Value);
    }
    else
    {
        /* Set Timer2 channel2 duty cycle */
...
static void LCD_CtrlLinesConfig( void )
{
    ...
    GPIO_Init( GPIOx_LCD_CTRL, &GPIO_InitStructure );

    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RS,  Bit_SET );    /* RS = 1   */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RD,  Bit_SET );    /* RD = 1   */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_CS,  Bit_SET );    /* CS = 1   */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_WR,  Bit_SET );    /* WR = 1   */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RST, Bit_RESET );  /* RST = 0  */
}
...
static void LCD_ST7565_Controller( void )
{
    ...
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RST, Bit_SET );    /* RST = 1  */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RST, Bit_RESET );  /* RST = 0  */
...
void LCD_SendLCDCmd( uint8_t Cmd )
{
    ...
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RS, Bit_RESET );     /* RS = 0 */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RD, Bit_SET );       /* RD = 1 */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_CS, Bit_RESET );     /* CS = 0 */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_WR, Bit_RESET );     /* WR = 0 */
...
void LCD_SendLCDData( uint8_t Data )
{
    ...
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RS, Bit_SET );
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RD, Bit_SET );
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_CS, Bit_RESET );
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_WR, Bit_RESET );

    /* Write data to the LCD */
    LCD_DataLinesWrite( GPIOx_LCD_DATA,(uint32_t)Data );
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_WR, Bit_SET );
}
...
uint8_t LCD_ReadLCDData( void )
{
    ...
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_WR, Bit_SET );         /* WR = 1 */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_CS, Bit_RESET );          /* CS = 0 */
    LCD_CtrlLinesWrite( GPIOx_LCD_CTRL, CtrlPin_RD, Bit_RESET );       /* RD = 0 */
...
/**
  * @brief  Displays one character (5dots width, 7dots height).
...
void LCD_DisplayChar(uint8_t Line, uint8_t Column, uint8_t Ascii)
{
    ...
    {
        LCD_SendLCDData(ASCII5x7_Table[((Ascii - 32) * FONT_WIDTH) + c]);
    }
}

/**
* @brief  Displays a maximum of 22 char on the LCD.
* @param  Line: the Line where to display the character shape .
*   This parameter can be one of the following values:
*     @arg Linex: where x can be 0..7
