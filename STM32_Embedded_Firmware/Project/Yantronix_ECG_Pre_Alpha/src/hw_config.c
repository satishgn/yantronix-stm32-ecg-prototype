...
void NVIC_Configuration(void)
{
  ...
  // /* Set the Vector Table base location at 0xB000 */ 
  ...
  /* Enable the EXTI9_5 Interrupt (UP & DOWN key) */
...
void MSD_Start(void)
{
    ...
    BKP_WriteBackupRegister(BKP_DR10, 0x4000);
#ifdef USE_USB_HID
    USB_Cable_Config(DISABLE);
#endif
    NVIC_SystemReset();
...
void VCP_Start(void)
{
    ...
    BKP_WriteBackupRegister(BKP_DR10, 0x8000);
#ifdef USE_USB_HID
    USB_Cable_Config(DISABLE);
#endif
    NVIC_SystemReset();
...
*/
    ...
    LCD_SetBackLight(LCD_BRIGHTNESS_MEDIUM);
...
void Delete_All_Log_Files(void)
{
    ...
    res = f_opendir(&dir, "/");
    if (res == FR_OK) {
        LCD_DisplayStringLine(LCD_LINE_4, "Deleting Log Files..");
...
void Draw_LCD_Trace(uint16_t *dataBuffer)
{
    ...
    for(Data_Channel_Index = 0; Data_Channel_Index < DATA_CHANNELS; Data_Channel_Index++, pWork++)
    {
        ...
        *pWork = *pWork>>6;    
        ...
        /* Draw data */
        LCD_DrawLine(traceOffset, prevData[Data_Channel_Index], traceOffset, *pWork, traceColor);
