...
uint16_t filter_offset = 2048;
// int32_t filterOffset = 0;

extern uint8_t Data_Channel_Filter_Enabled[DATA_CHANNELS];
extern __IO uint8_t Data_Channel_Index;

#ifdef FLOATING_POINT_PROCESS
/*************** Floating point Process ****************/
void iir_mk_filter(uint16_t* iir_output, uint16_t* iir_input)
{
    for(Data_Channel_Index = 0; Data_Channel_Index < DATA_CHANNELS; Data_Channel_Index++, iir_input++, iir_output++)
    {
        if(!Data_Channel_Filter_Enabled[Data_Channel_Index])
        {
            *iir_output = *iir_input;
            continue;
        }
...
corner  =  10 Hz
*/
uint16_t qrs_filter(uint16_t filter_input)
{
    ...
    return (uint16_t)(yv_qrs[4] + filter_offset);
}