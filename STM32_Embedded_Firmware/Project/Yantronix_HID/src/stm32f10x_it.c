...
extern uint16_t AUTO_SHUTDOWN_ENABLED;

#ifdef USE_IWDG 
__IO uint32_t IWDG_ReloadCounter_Time = 0;
#endif
...
void NMI_Handler(void)
{
  ...
  { 
    ...
    GPIO_SetBits(GPIO_LED, GPIO_LED1_PIN | GPIO_LED2_PIN);
    ...
    LCD_DrawRect(Line3-1, 0, CHAR_HEIGHT+2, SCREEN_WIDTH);
    ...
    while(RCC_GetFlagStatus(RCC_FLAG_HSERDY) == RESET)
    {
      if(index == 0x3FFFF)
      {
        if(tmp > SCREEN_WIDTH - CHAR_WIDTH - 1)
             tmp = SCREEN_WIDTH - CHAR_WIDTH - 1;      
        LCD_DisplayChar(Line3, tmp, 0x20);  //ASCII 32 -> Space
        tmp += CHAR_WIDTH;    
        index = 0;
      }
      index++;
...
{
#ifdef USE_IWDG 
    IWDG_ReloadCounter_Time += 10;
    if(IWDG_ReloadCounter_Time >= 10000)    //10 sec
    {
        // /* Toggle LED1 */
        // GPIO_WriteBit(GPIO_LED, GPIO_LED1_PIN, (BitAction)((1-GPIO_ReadOutputDataBit(GPIO_LED, GPIO_LED1_PIN))));
        /* Reload IWDG counter every 10 sec */
        IWDG_ReloadCounter();
        IWDG_ReloadCounter_Time = 0;  
    }
#endif
